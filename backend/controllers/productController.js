...
try {
    ...
    const images = [image1, image2, image3, image4].filter((item)=> item !== undefined)
     ...
     const productData = {
        ...
        sizes: sizes ? JSON.parse(sizes) : [], ///default to empty if sizes are not provided
        image: imagesUrl,
        date: Date.now()
     }
     ...
     res.json({success:true, message:"Product Added Bruv!"})
} catch (error) {
    console.log(error)
...
try {
    ...
    res.json({success:true, message:"Product Removed Bruv!"})
} catch (error) {
    console.log(error)
    res.json({success:false, message:error.message})
}
...
// controller function for single products
